...
"""Tests for `kombu_cracker` package."""
...
def _nobuf(x):
    return [str(i) if isinstance(i, bytes) else i for i in x]
...
class TransportCase(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        kombu_cracker.register()

    @classmethod
    def tearDownClass(cls):
        kombu_cracker.unregister()
    ...
    def test_produce__consume_multiple(self):
        ...
        self.assertListEqual(_nobuf(messages), ['b1', 'b2', 'b3'])
        chan2.close()
